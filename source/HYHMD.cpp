...
HyHMD::HyHMD(std::string id, HyDevice* Device) {
	...
	m_pFrameCoder = new FrameCoder(m_pDispHandle, m_pD3D11Device);
...
bool HyHMD::copyToStaging()
{
	...
	m_pD3D11DeviceContext->CopyResource(m_pStagingTexture, m_pTexture);
	//DriverLog("Copyed");
	return true;
}
